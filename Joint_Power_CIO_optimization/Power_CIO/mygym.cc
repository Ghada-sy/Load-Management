...
MyGymEnv::MyGymEnv (double stepTime, uint32_t N1, uint32_t N2, uint16_t N3)
{
  ...
  block_Thr=0.5;//Mb/s
  m_chooseReward = 1;//0 average cell throuput, 2 number of blocked users
  m_interval = stepTime;
  m_cellCount = N1;
  m_userCount = N2;
...
MyGymEnv::GetActionSpace()
{
  ...
  float low = 6.0;//cio action
  float high = 3.0;//power action
  std::vector<uint32_t> shape = {11+m_cellCount,};
...
MyGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  std::vector<double> energy_actionVectord(actionVector.begin()+11, actionVector.end());
  ...
  CellIndividualOffset::setOffsetList(cio_actionVectord);     //execute CIO action
	for(size_t it = 0; it != m_cells.GetN(); ++it)
	{
		...
		enbPhy->SetTxPower(32 + energy_actionVectord[it]);      //execute energy action
 ...
 //int mimo_actionVectord[6]={3,3,3,3,3,3};   //MIMO ON
 int mimo_actionVectord[6]={1,1,1,1,1,1};   //MIMO OFF
	
	for(size_t it = 0; it != m_cells.GetN(); ++it)
	
	{
		ns3::Ptr<ns3::NetDevice> netDevice = m_cells.Get(it);//get network device object of it+1 cell
		...
		Ptr<PfFfMacScheduler> pfsched = ptrval.Get<PfFfMacScheduler> ();//define a pointer pfsched that points to the RrFfMacScheduler of FfMacScheduler
		
		std::map<uint16_t, float> tempmap=UserCQI [it+1];// to get the map that contain the rnti of the users in cell it+1
		std::map<uint16_t, float>::iterator ptr;
		
		for (ptr = tempmap.begin(); ptr != tempmap.end(); ptr++)//users loop
		
		{
			pfsched ->TransmissionModeConfigurationUpdate(ptr->first, mimo_actionVectord[it]);//itr->first is the rnti    //execute MIMO action
		}
